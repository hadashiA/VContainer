...
:::caution
Currently, this feature requires Unity 2022.2.0b12 or later versions.
:::

## Setup

ECS features for VContainer is enabled if the project has the `com.unity.entities.graphics` package installed.
...
If the `com.unity.entities.graphics` package exists, the `VCONTAINER_ECS_INTEGRATION` symbol is defined and the following features are enabled.

:::caution
Currently, the flag does not ensure that the correct versions of 'com.unity.entities' and 'com.unity.entities.graphics' are installed...
The code has been written against version '1.0.0-exp.08' and tested up to versions 'com.unity.entities' '1.0.0-exp.12' and 'com.unity.entities.graphics' '1.0.0-exp.14'
:::
