...
通常、Unityでアプリケーションとかゲームを書くには、Unityのライフサイクルイベント上でコードを実行しなければいけないはずです。

通常、Unityでは MonoBehaviour の `Start` / `Update` / `OnDestroy` などがそれに相当します。

このような処理の起点のことを、VContainerでは 「EntryPoint」と呼んでいます。
