...
:::note
If `VCONTAINER_UNITASK_INTEGRATION` is **not** enabled (e.g. UniTask package is missing), `IAsyncStartable` falls back to `System.Threading.Tasks.Task` or `UnityEngine.Awaitable`. However, in Unity 2020 or older, `IAsyncStartable` is ignored without UniTask integration.
:::
...
For more information about `RegisterEntryPoint`, see [here](../integrations/entrypoint.mdx).
...
See the [Entry point](./entrypoint.mdx) section for more information.
...
See the [Scoping](../scoping/generate-child-via-scene.mdx) section for more information.
