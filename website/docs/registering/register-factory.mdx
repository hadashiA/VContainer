...
VContainer generally constructs registered dependencies the first time they're resolved (except for [registered instances](./register-instance.mdx#register-instance)). If you need finer control over when a dependency is created, you can register and use a factory function.
...
In the following example, dependency resolution will happen only once. Dependency resolution will not occur automatically in this particular `Create()` method (that is what the constructor is for), although you can explicitly use the [`IObjectResolver` API](../resolving/container-api.mdx) within a factory if you wish.
...
This version requires a [`Lifetime`](../scoping/lifetime-overview.mdx) that specifies how often the inner `Func<>` is generated, i.e. how often the outer `Func<>` is called.
...
See the [Container API page](../resolving/container-api.mdx) for more information.
...
Factories can be registered as any delegate that can be converted to a `Func<>`, including methods. This way, all factories can be used as `Func<>`s no matter how complicated the underlying implementation is.
...
While `Func<...>` is convenient, sometimes you may want to define a concrete factory interface and implementation for better testability or to encapsulate complex creation logic.

```csharp
public interface IEnemyFactory
{
    Enemy Create(string id, Vector3 position);
}
...
public class EnemyFactory : IEnemyFactory
{
    ...
    public EnemyFactory(IObjectResolver container, EnemyData data)
    {
    ...
    {
        var prefab = data.GetPrefab(id);
        // Use Instantiate to inject dependencies into the new enemy instance
        var enemy = container.Instantiate(prefab, position, Quaternion.identity);
        return enemy;
    }
}
```
...
public class GameLoop
{
    ...
    public GameLoop(IEnemyFactory enemyFactory)
    {
    ...
    public void Spawn()
    {
        var enemy = enemyFactory.Create("boss", Vector3.zero);
    }
}
```
...
If you need to perform asynchronous operations (like loading assets) within a factory, you can register a factory that returns a `UniTask<T>` (or `Task<T>`).

```csharp
// Example using Cysharp.Threading.Tasks (UniTask)
builder.RegisterFactory<string, UniTask<Texture>>(container =>
{
    return async key =>
    {
        // Example: Async load via Addressables
        var texture = await Addressables.LoadAssetAsync<Texture>(key);
        return texture;
    };
}, Lifetime.Scoped);
```

You can inject and use it as follows:

```csharp
class TexturePresenter
{
    readonly Func<string, UniTask<Texture>> textureFactory;

    public TexturePresenter(Func<string, UniTask<Texture>> textureFactory)
    {
    ...
    {
        var texture = await textureFactory(key);
        // Use texture...
    }
}
```
