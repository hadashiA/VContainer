...
class SceneLoader
{
    ...
    async UniTask LoadAddressableSceneAsync(string key)
    {
        // LifetimeScope.EnqueueParent works with Addressables too
        using (LifetimeScope.EnqueueParent(parent))
        {
            await Addressables.LoadSceneAsync(key, LoadSceneMode.Additive);
        }
    }
}
```
...
Often, you may want to add additional registrations to the loaded scenes.
