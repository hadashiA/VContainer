...
- When playing the scene, LifetimeScope automatically builds the Container and dispatches to its own PlayerLoopSystem.
...
You instruct the `builder` to register the class above.
...
**3. Create GameObject and attach your LifetimeScope**

Right Click inside the Hierarchy tab and select **Create Empty**, and name it "GameLifetimeScope".
...
As such, it's good practice to keep any side-effect entry points within the marker interface.

(By design, MonoBehaviour's Start/Update methods are sufficient for simple cases. However, VContainer's marker interfaces serve to separate the entry point of domain logic from presentation logic.)
...
- **HelloWorldService:** Responsible only for functionality that can be called anytime, anywhere.
- **HelloScreen**: Responsible only for the View.
...
Now that you understand the basics, we highly recommend reading the [Best Practices](../best-practices) guide to learn how to structure your project effectively and avoid common pitfalls.
