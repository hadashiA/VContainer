...
In general, dependencies are constructed and injected when they're resolved for the first time (with [some](../registering/register-factory.mdx) [exceptions](../registering/register-instance.mdx#register-instance)).
...
:::note
The `Key` attribute must be used with types that have been registered with the corresponding key using the `.Keyed()` method. See the [Register with Keys](../registering/register-with-keys.mdx) section for more information.
:::
